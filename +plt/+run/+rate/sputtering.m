function [fig] = sputtering(output, input, options)
%SPUTTERING plot the rates at which species are sputtered from the target
% =========================================================================
...
options.Mask = util.mask.op([input.Range.sput_metal, input.Range.sput_gas], @intersect);

options.YLabel = '$\frac{{\rm d} n}{{\rm d} t}$ [$\rm m^{-3}\,s^{-1}$]';
options.XLabel = '$t$ [$\rm \mu s$]';

options_ = namedargs2cell(options);
fig = plt.run.util.perSpeciesProperty(output, input, 'sputrate', options_{:});

if ~any(isnan(options.XLim))
    xlim(options.XLim);
elseif isfield(input.disch, 't_pulse_end')
    xlims = xlim();
    xlims(2) = util.num.roundTo(input.disch.t_pulse_end, xticks(), 'Direction', 'up');
    xlim(xlims);
end

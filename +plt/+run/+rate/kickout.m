function [fig] = kickout(output, input, options)
%KICKOUT plot the rates at which species are kicked out from the IR
% =========================================================================
% Plots the rate of change of the densities of the different species due to
% kickout/sputter wind.
...
options.Mask = util.mask.op(input.Range.kickout, @intersect);

options.YLabel = '$\frac{{\rm d} n}{{\rm d} t}$ [$\rm m^{-3}\,s^{-1}$]';
options.XLabel = '$t$ [$\rm \mu s$]';

options_ = namedargs2cell(options);
fig = plt.run.util.perSpeciesProperty(output, input, 'kickout', options_{:});

set(gca, 'YScale', 'log');

if any(isnan(options.YLim))
   ylims = ylim();
   ylims(1) = max(ylims(1), 1e+10);
   ylim(ylims);
end

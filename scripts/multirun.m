...
% Results folder
path_results    = fullfile('results','ArSi_Zr'); % path where the simulation output will be written to
% Parameter grid
Pfit.names      = {'f','beta','r'}; % don't change this
Pfit.p{1}       = 0.12:0.01:0.17; % parameter range for f (IR potential drop fraction)
Pfit.p{2}       = 0.7:0.01:.8; % parameter range for beta (back-attraction probability)
Pfit.p{3}       = 0.7:0.1:0.7; % parameter range for r (electron recapture probability)
% Discharge properties0.74
F_flux_meas     = 0.2548; % measured ionized flux fraction, can also be specified directly in the disch structure 

...
gamma_clean_metal_Ar  = @(E) 1e-5*(E > 500).*(E - 500).^(1.2)./(1 + (E/70000).^(0.7)); % B12 Phelps
...
gamma_clean_metal_Arii_baragiola = @(E) 0.032*(0.78*27.63-2*4.73); % Baragiola Arii

gamma_clean_metal_Ari_hagstrum = @(E) 5.640165e-04 * sqrt(E) + 2.138111e-02; % Hagstrum 
...
if ismember('Ar',Names)
    ...
    fGamma.C{s.Ari} = @(Ud) gamma_clean_metal_Ari_hagstrum(Ud); %
    if ismember('Arii',Names)
        ...
        fGamma.C{s.Arii} = @(Ud) gamma_clean_metal_Arii_baragiola(Ud);
    end
end

function [filename, Ec] = create_Ec_ion_cost_Si(Spe, name, generate_plots)
...
% -------------------------------------------------------------------------
% USAGE EXAMPLE:
% create_Ec_ion_cost_Si(Spe, 'Ec_ArTi')
%==========================================================================
...
%    Tec_max = 15.8;  % temperature max in eV
%end
...
if ismember("Ar", Spe.Names)
    ...
    % cold
    fEc.c{s.Ar} = @(T) Ecc_Cal_Ar_2(T);
    fEc.c{s.ArW} = @(T) fEc.c{s.Ar}(T);
    fEc.c{s.ArH} = @(T) fEc.c{s.Ar}(T);
    fEc.c{s.Arm3P0} = @(T) Spe.Energy(s.Ari) - Spe.Energy(s.Arm3P0);
    fEc.c{s.Arm3P2} = @(T) Spe.Energy(s.Ari) - Spe.Energy(s.Arm3P2);
    fEc.c{s.Ari} = @(T) Spe.Energy(s.Arii);
 %   fEc.c{s.Arii} = @(T) Spe.Energy(s.Ariii);
    
    % hot
    EchAr   = 21.95; %

    fEc.h{s.Ar} = @(T) Ecc_Cal_Ar_2(T);
    fEc.h{s.ArW} = @(T) fEc.h{s.Ar}(T);
    fEc.h{s.ArH} = @(T) fEc.h{s.Ar}(T);
    fEc.h{s.Arm3P0} = @(T) fEc.c{s.Arm3P0}(T);
    fEc.h{s.Arm3P2} = @(T) fEc.c{s.Arm3P2}(T);
    fEc.h{s.Ari} = @(T) fEc.c{s.Ari}(T);
 %   fEc.h{s.Arii} = @(T) fEc.c{s.Arii}(T);
end
...
if ismember("Si", Spe.Names)
    ...
    fEc.c{s.Sii} = @(T) EizSi;
  %  fEc.c{s.Siii} = @(T) EizSii;
    
    % hot
    fEc.h{s.Si} = @(T) sixx(T,Teh_min,dTeh,Teh_max);
    fEc.h{s.Sii} = @(T) EizSi;
  %  fEc.h{s.Siii} = @(T) EizSii;
